...
def load_model_and_tokenizer():
    """
    Handles authentication and loads the Llama-3.2-3B model in 16-bit precision.
    """
    ...
    print(f"Loading model: {MODEL_ID} in 16-bit (bfloat16) precision...")
    ...
    if tokenizer.chat_template is None:
        print("Tokenizer missing chat template. Applying Llama 3 template manually.")
...
def parse_single_response(response_text):
    """
    Robustly parses a single model response text, defaulting to 0 if parsing fails.
    """
    prediction = 0
    ...
    if re.search(r'REGIONAL BIAS|\b1\b', response_text, re.IGNORECASE):
        prediction = 1
    elif re.search(r'NON-REGIONAL BIAS|\b0\b', response_text, re.IGNORECASE):
        prediction = 0
    else:
        print(f"Warning: Could not reliably parse classification. Defaulting to 0. Response: '{response_text}'")
        
    return prediction, reasoning

def classify_batch(comments, model, tokenizer):
    """
    Generates classifications for a batch of comments using Llama's chat template.
    """
    messages = [
        [
...
def generate_evaluation_outputs(df):
    ...
    """
    if df.empty:
        print("The dataframe is empty. Skipping evaluation.")
        return
    ...
    with open(report_path, "w") as f:
        f.write(f"Classification Report for model: {MODEL_ID} (16-bit, Original Prompt)\n\n")
        f.write(report)
    ...
    plt.title(f'Confusion Matrix - {MODEL_ID} (16-bit, Original Prompt)')
